...
#define ALLOCATE_OBJ(type, objectType, objectClass)                            \
  (type *)allocateObject(vm, sizeof(type), objectType, objectClass,            \
                         GC_GENERATION_TYPE_EDEN)
#define ALLOCATE_OBJ_GEN(type, objectType, objectClass, generation)            \
  (type *)allocateObject(vm, sizeof(type), objectType, objectClass, generation)
#define ALLOCATE_CLASS(classClass)                                             \
  ALLOCATE_OBJ_GEN(ObjClass, OBJ_CLASS, classClass,                            \
                   GC_GENERATION_TYPE_PERMANENT)
#define ALLOCATE_CLOSURE(closureClass, generation)                             \
  ALLOCATE_OBJ_GEN(ObjClosure, OBJ_CLOSURE, closureClass, generation)
#define ALLOCATE_NAMESPACE(namespaceClass)                                     \
  ALLOCATE_OBJ_GEN(ObjNamespace, OBJ_NAMESPACE, namespaceClass,                \
                   GC_GENERATION_TYPE_PERMANENT)
...
#define IS_BOOL_INSTANCE(arg)                                                  \
  (IS_BOOL(arg) ||                                                             \
   (IS_VALUE_INSTANCE(arg) && IS_BOOL(AS_VALUE_INSTANCE(arg)->value)))
...
#define IS_FLOAT_INSTANCE(arg)                                                 \
  (IS_FLOAT(arg) ||                                                            \
   (IS_VALUE_INSTANCE(arg) && IS_FLOAT(AS_VALUE_INSTANCE(arg)->value)))
...
#define IS_INT_INSTANCE(arg)                                                   \
  (IS_INT(arg) ||                                                              \
   (IS_VALUE_INSTANCE(arg) && IS_INT(AS_VALUE_INSTANCE(arg)->value)))
...
#define IS_NIL_INSTANCE(arg)                                                   \
  (IS_NIL(arg) ||                                                              \
   (IS_VALUE_INSTANCE(arg) && IS_NIL(AS_VALUE_INSTANCE(arg)->value)))
#define IS_NODE(value) isObjType(value, OBJ_NODE)
#define IS_NUMBER_INSTANCE(arg)                                                \
  (IS_NUMBER(arg) ||                                                           \
   (IS_VALUE_INSTANCE(arg) && IS_NUMBER(AS_VALUE_INSTANCE(arg)->value)))
...
#define AS_BOOL_INSTANCE(arg)                                                  \
  (IS_BOOL(arg) ? AS_BOOL(arg) : AS_BOOL(AS_VALUE_INSTANCE(arg)->value))
...
#define AS_FLOAT_INSTANCE(arg)                                                 \
  (IS_FLOAT(arg) ? AS_FLOAT(arg) : AS_FLOAT(AS_VALUE_INSTANCE(arg)->value))
#define AS_FRAME(value) ((ObjFrame *)AS_OBJ(value))
#define AS_FUNCTION(value) ((ObjFunction *)AS_OBJ(value))
#define AS_GENERATOR(value) ((ObjGenerator *)AS_OBJ(value))
#define AS_INSTANCE(value) ((ObjInstance *)AS_OBJ(value))
#define AS_INT_INSTANCE(arg)                                                   \
  (IS_INT(arg) ? AS_INT(arg) : AS_INT(AS_VALUE_INSTANCE(arg)->value))
...
#define AS_NUMBER_INSTANCE(arg)                                                \
  (IS_NUMBER(arg) ? AS_NUMBER(arg) : AS_NUMBER(AS_VALUE_INSTANCE(arg)->value))
...
typedef Value (*NativeMethod)(VM *vm, Value receiver, int argCount,
                              Value *args);
...
void initNamespace(VM *vm, ObjNamespace *namespace, ObjString *shortName,
                   ObjNamespace *enclosing);
ObjNamespace *newNamespace(VM *vm, ObjString *shortName,
                           ObjNamespace *enclosing);
ObjNativeFunction *newNativeFunction(VM *vm, ObjString *name, int arity,
                                     bool isAsync, NativeFunction function);
ObjNativeMethod *newNativeMethod(VM *vm, ObjClass *klass, ObjString *name,
                                 int arity, bool isAsync, NativeMethod method);
...
void copyObjProperty(VM *vm, ObjInstance *object, ObjInstance *object2,
                     char *name);
